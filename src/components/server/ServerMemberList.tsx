  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: profiles } = await supabase
        .from("profiles")
        .select(
          "user_id, display_name, username, avatar_url, banner_url, about_me, status, created_at, name_gradient_start, name_gradient_end",
        )
        .in("user_id", userIds);

      const profileMap = new Map((profiles || []).map((p) => [p.user_id, p]));
      setMembers((data as any[]).map((m) => ({ ...m, profile: profileMap.get(m.user_id) })));
      setLoading(false);
    };
    load();
    ...
    <div className="w-[240px] flex flex-col bg-sidebar-background border-s border-sidebar-border shrink-0 overflow-hidden">
      <div className="p-3 border-b border-sidebar-border">
        <h3 className="text-xs font-semibold uppercase text-muted-foreground">
          {t("servers.members")} — {members.length}
        </h3>
      </div>
        ...
        {loading ? (
          ...
          <div className="animate-fade-in space-y-4">
            {sortedGroups.map(([label, mems]) => (
              <div key={label}>
                <span className="text-[11px] font-semibold uppercase text-muted-foreground px-1">
                  {label} — {mems.length}
                </span>
                      ...
                      <div className="overflow-hidden rounded-lg bg-popover">
                        ...
                        <div className="px-4 pb-3">
                          <div className="relative -mt-8 mb-2">
                            ...
                            <StatusBadge
                              status={(status === "offline" ? "invisible" : status) as UserStatus}
                              size="md"
                              className="absolute bottom-1 start-12"
                            />
                          </div>
                          ...
                          <Badge
                            className={`mt-2 text-[10px] px-2 py-0.5 ${roleBadgeColors[m.role] || roleBadgeColors.member}`}
                          >
                            {t(`servers.${m.role}`)}
                          </Badge>
                    ...
                    return (
                      <Popover key={m.user_id}>
                        <ServerMemberContextMenu
                          ...
                          currentUserRole={currentUserRole}
                        >
                          <PopoverTrigger asChild>{memberButton}</PopoverTrigger>
                        </ServerMemberContextMenu>
